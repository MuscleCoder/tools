...
###################################################################################################################
# This script is from https://github.com/chadoe/docker-cleanup-volumes, I add some function the clean the docker
# more.
###################################################################################################################
...
function delete_overlay() {
  echo "Start remove exited docker containers"
  docker ps -f status=exited -q | xargs -r docker rm
  echo "Start remove dangling docker images"
